...
describe("getters", () => {
  ...
  it("test filter accounts by name", () => {
    const empty = getters.filterAccountsByName(testState, null, rootState, null)("")

    expect(empty).toEqual([])

    const test = getters.filterAccountsByName(testState, null, rootState, null)("test")

    expect(test).toEqual(testState.accounts)

    const test2 = getters.filterAccountsByName(testState, null, rootState, null)("test2")

    expect(test2).toEqual([testState.accounts[1]])
  })
  it("test filter accounts by user id", () => {
    const empty = getters.filterAccountsByUserId(testState, null, rootState, null)(3)

    expect(empty).toEqual([])

    const actual = getters.filterAccountsByUserId(testState, null, rootState, null)(2)

    expect(actual).toEqual([testState.accounts[1]])
  })
  it("test filter accounts by user id and name", () => {
    const empty1 = getters.filterAccountsByUserIdAndName(testState, null, rootState, null)(3, "test")
    const empty2 = getters.filterAccountsByUserIdAndName(testState, null, rootState, null)(1, "test2")

    expect(empty1).toEqual([])
    expect(empty2).toEqual([])

    const actual = getters.filterAccountsByUserIdAndName(testState, null, rootState, null)(1, "test")
