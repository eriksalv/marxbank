...
export class RestService {
  ...
  public setToken(token: string | null) {
    if (token === null)
      delete this.handler.defaults.headers.common["Authorization"];
    else
      this.handler.defaults.headers.common["Authorization"] = `Bearer:${token}`;
  }

  // hente GETS
  public get<T>(
    url: string,
    config?: { params?: AxiosRequestHeaders; headers?: AxiosRequestHeaders }
  ): Promise<T> {
  ...
  // POSTS
  public post(url: string, data?: object | null): Promise<void> {
    return promiseWrapper(
      this.handler.post(url, data, {
  ...
  public put(url: string, data?: object): Promise<void> {
    return promiseWrapper(
      this.handler.put(url, data, {
        validateStatus: (code: number) => code === 200 || code === 204,
      })
    );
  }
